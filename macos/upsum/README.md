...
```bash
uv run upsum --log-dir ~/Library/Logs/UpdateAll/
```
...
<dict>
    ...
    <array>
        ...
        <string>-lc</string>
        <string>cd /Users/YOUR_USERNAME/git/Scripts/macos/upsum && uv run upsum</string>
    </array>
...
```crontab
# 매일 새벽 4시에 upsum 실행
0 4 * * * cd $HOME/git/Scripts/macos/upsum && uv run upsum > /private/tmp/upsum_cron.log 2>&1
```

**참고:** `crontab`에서 `uv run`을 실행하려면 프로젝트 디렉토리로 이동한 뒤 실행하는 것이 가장 안정적입니다. `which uv` 명령어로 `uv`의 설치 경로를 확인할 수 있습니다.